...
class DualECAAttention(nn.Module):
    ...
    def forward(self, x):
        y_gap = self.gap(x)
        y_gap = y_gap.squeeze(-1).permute(0, 2, 1)
        y_gap = self.conv_gap(y_gap)
        y_gmp = self.gmp(x)
        y_gmp = y_gmp.squeeze(-1).permute(0, 2, 1)
        y_gmp = self.conv_gmp(y_gmp)
        y_fused = y_gap + y_gmp
        y_fused = self.sigmoid(y_fused)
        y_fused = y_fused.permute(0, 2, 1).unsqueeze(-1)
...
class LeafBModule(nn.Module):
    def __init__(self, dim,
        ...
        if current_total != dim:
            ...
            if self.conv_channels > 0:
                self.conv_channels += diff
            elif self.wt_enh_channels > 0:
                self.wt_enh_channels += diff
            elif self.identity_channels > 0:
                self.identity_channels += diff
            else:
                if dim > 0:
    ...
    def forward(self, x):
        ...
        if self.conv_channels > 0:
            x_conv = self.se_conv(x_conv)
        if actual_wt_enh_channels > 0:
            x_wt_enh = self.se_wt_enh(x_wt_enh)
        outputs_to_cat = []
        if self.identity_channels > 0:
            outputs_to_cat.append(x_identity)
        if self.conv_channels > 0:
            outputs_to_cat.append(x_conv)
        if actual_wt_enh_channels > 0:
            outputs_to_cat.append(x_wt_enh)
...
class Stem(nn.Module):
    def __init__(self, in_channels, stem_inter_channels, out_channels):
        ...
        self.conv2 = BasicConv(stem_inter_channels, stem_inter_channels, kernel_size=3, stride=2, padding=1, relu=True, bn=True)
        self.conv3 = BasicConv(stem_inter_channels, out_channels, kernel_size=3, stride=2, padding=1, relu=False, bn=True)
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
...
class DownsampleModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.dw_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=2, padding=1, groups=in_channels, bias=False)
...
class StageBlock(nn.Module):
    def __init__(self, dim, core_module_class,
        ...
        if core_module_class == LeafBModule:
            lb_defaults = {
...
def count_dual_eca_attention(m, x_tuple, y):
    ...
    kernel_size = m.conv_gap.kernel_size[0]
    m.total_ops += 2 * (C * kernel_size)
    m.total_ops += C
    m.total_ops += C
    m.total_ops += x_input.nelement()

def create_LeafB_Net_L():
...
def create_LeafB_Net_S():
    config_embed_dims = [32, 64, 96, 128]
    config_depths = [0, 1, 3, 3]
    num_classes = 1000

...
# 默认尝试使用 Triton
_REAL_TRITON_AVAILABLE = False
_TRITON_MODULE = None
_TRITON_LANGUAGE_MODULE = None

try:
    import triton as _real_triton_module_source  # 使用不同的别名导入真正的Triton
    import triton.language as _real_triton_language_module_source

    # 进行一个简单的检查，确认导入的是真正的 Triton 而不是我们的假模块
    # (例如，检查一个真实 Triton 才有的特定属性或函数类型)
    if hasattr(_real_triton_module_source, 'compile') and callable(_real_triton_module_source.compile):
        _REAL_TRITON_AVAILABLE = True
        _TRITON_MODULE = _real_triton_module_source
        _TRITON_LANGUAGE_MODULE = _real_triton_language_module_source
        print("Successfully imported REAL Triton.")
    else:
        warnings.warn(
            "Imported a module named 'triton', but it does not appear to be the real Triton library. Falling back.")
        # 如果导入的 'triton' 不是我们期望的，也视为不可用
except ImportError:
    warnings.warn("Triton not installed, fall back to PyTorch implementations.")

# 全局的 WITH_TRITON 标志，现在更可靠
WITH_TRITON = _REAL_TRITON_AVAILABLE

# to make sure cached_property can be loaded for triton
if WITH_TRITON:  # 只有当真正的 Triton 可用时
    try:
        from functools import cached_property
    except ImportError:  # 更具体的异常
...
if WITH_TRITON:
    # 使用我们导入的真实 Triton 模块
    # 这些 tl 和 triton 变量现在指向真实的 Triton
    tl = _TRITON_LANGUAGE_MODULE
    triton = _TRITON_MODULE
    @triton.jit
...
else: # 如果 WITH_TRITON 为 False (即真正的 Triton 不可用)
    # 定义占位符或确保这些类不会被引用
    # cross_scan_fn 和 cross_merge_fn 会自动选择 PyTorch 的 CrossScanF/CrossMergeF
    # 所以这里不需要显式定义 CrossScanTritonF 和 CrossMergeTritonF 的替代品
    # 只要确保它们在 WITH_TRITON=False 时不会被 cross_scan_fn/cross_merge_fn 错误地选中即可
    pass

# @torch.compile(options={"triton.cudagraphs": True}, fullgraph=True)
# cross_scan_fn 和 cross_merge_fn 的选择逻辑保持不变，但现在它们依赖于更可靠的 WITH_TRITON
def cross_scan_fn(x: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0, force_torch=False):
    if WITH_TRITON and x.is_cuda and (not force_torch):
        # 确保 CrossScanTritonF 只在 WITH_TRITON 为 True 时被引用
        # 之前的 if WITH_TRITON: 包裹已经保证了这一点
        CSF_selected = CrossScanTritonF
    else:
        CSF_selected = CrossScanF
    return CSF_selected.apply(x, in_channel_first, out_channel_first, one_by_one, scans)
...
def cross_merge_fn(y: torch.Tensor, in_channel_first=True, out_channel_first=True, one_by_one=False, scans=0, force_torch=False):
    if WITH_TRITON and y.is_cuda and (not force_torch):
        CMF_selected = CrossMergeTritonF
    else:
        CMF_selected = CrossMergeF
    return CMF_selected.apply(y, in_channel_first, out_channel_first, one_by_one, scans)

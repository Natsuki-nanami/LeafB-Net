...
from VMRNN_self.vmamba_2 import SS2D


# from model.modelCode.MobileMamba.model.lib_mamba.vmamba import SS2D
# from model.MobileMamba import MODEL

# from model.modelCode.MobileMamba.util.util import FLOPs
...
class MBWTConv2d(nn.Module):
    ...
    def forward(self, x): # x 的输入形状是 (B, self.in_channels, H, W)
    
            # --- Wavelet transform part (lines 88-128) ---
            # This part calculates x_tag. The original x remains (B, self.in_channels, H, W)
            x_ll_in_levels = []
            x_h_in_levels = []
            shapes_in_levels = []
    
            curr_x_ll = x # curr_x_ll starts as the input x
            ...
            for i in range(self.wt_levels):
                ...
                curr_x_tag_intermediate = curr_x.reshape(shape_x[0], shape_x[1] * 4, shape_x[3], shape_x[4]) # Renamed to avoid confusion
            ...
            for i in range(self.wt_levels - 1, -1, -1):
                curr_x_ll_from_list = x_ll_in_levels.pop() # Renamed to avoid confusion
                curr_x_h = x_h_in_levels.pop()
                curr_shape = shapes_in_levels.pop()
    
                curr_x_ll_from_list = curr_x_ll_from_list + next_x_ll # curr_x_ll here is from the list
    
                curr_x_combined = torch.cat([curr_x_ll_from_list.unsqueeze(2), curr_x_h], dim=2) # Renamed
                next_x_ll = self.iwt_function(curr_x_combined)
    
                next_x_ll = next_x_ll[:, :, :curr_shape[2], :curr_shape[3]]
    
            x_tag = next_x_ll # x_tag is (B, self.in_channels, H, W)
            assert len(x_ll_in_levels) == 0
    
            # --- Global attention path ---
            # The input 'x' to this MBWTConv2d.forward method is (B, self.in_channels, H, W)
            # SS2D (self.global_atten) expects (B, H, W, self.in_channels)
            
            # 1. Permute x for SS2D
            x_for_ss2d = x.permute(0, 2, 3, 1).contiguous()  # Shape: (B, H, W, self.in_channels)
            
            # 2. Pass to SS2D
            # SS2D.forward will return a tensor of shape (B, H, W, self.d_model)
            # where self.d_model is self.in_channels for this SS2D instance
            output_from_ss2d = self.global_atten(x_for_ss2d) 
            
            # 3. Permute back to (B, self.in_channels, H, W) for subsequent operations
            x_global_path = output_from_ss2d.permute(0, 3, 1, 2).contiguous()
            
            # 4. Apply scaling
            x_global_path = self.base_scale(x_global_path)
    
            # Combine global path with wavelet path (x_tag)
            # Both x_global_path and x_tag should now be (B, self.in_channels, H, W)
            output = x_global_path + x_tag # 'output' is the new name for the combined result
...
# MobileMamba.py, in if __name__ == '__main__':
...
if __name__ == '__main__':
    ...
    print("\n--- Performing a forward pass on modified LBNet_test2 ---")
    try:
    ...
    print("\n--- Modified LBNet_test2 Summary ---")
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total trainable parameters: {total_params / 1e6:.2f} M")

    model.eval()
    macs, params = profile(model, inputs=(thop_input_tensor,), custom_ops={})
    macs, params = clever_format([macs, params], "%.3f")

    print('Flops:  ', macs)
    print('Params: ', params)

    throughput_dummy_input = torch.randn(64, 3, 224, 224).to(device)
